  ...
  return (
    ...
    >
      {imageSrc ? (
        ...
        <div className="flex h-full w-full items-center justify-center rounded-lg border-3 border-solid border-neutral-500 bg-neutral-200">
      ...
      <div className="transition-filter absolute inset-0 flex h-full w-full items-end rounded-lg bg-gradient-to-b from-transparent to-neutral-800 opacity-0 duration-300 ease-out group-hover:opacity-100">
        <div onClick={handleClickLike}>
          {isLiked ? (
            <HeartIcon className="absolute top-2.75 right-3 z-10 h-12 w-12 fill-fuchsia-500 text-fuchsia-800" />
          ) : (
            <HeartIcon className="absolute top-2.75 right-3 z-10 h-12 w-12 fill-neutral-100 text-neutral-500" />
          )}
        </div>
